...
SELECT
    ...
    SUM(COALESCE(precipitation_last_hour_mm, 0))     AS total_precipitation_mm,
...
{% if is_incremental() %}
-- Reprocess yesterday and today so late-arriving observations correctly update
-- yesterday's aggregate without touching historical dates
WHERE DATE(observation_timestamp) >= DATE_SUB(CURRENT_DATE(), INTERVAL 1 DAY)
{% endif %}
